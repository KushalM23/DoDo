...
export function TasksProvider({children}: {children: React.ReactNode}) {
  ...
  const toggleTaskCompletion = useCallback(
    async (task: Task) => {
      ...
      const updated = await updateTaskLocal(user.id, task.id, {completed: newCompleted});
      if (updated) {
  ...
  const startTimer = useCallback(
    async (task: Task) => {
      ...
      const updated = await updateTaskLocal(user.id, task.id, {timerStartedAt: now});
      if (updated) {
        setTasks(prev => sortTasks(prev.map(t => (t.id === task.id ? updated : t)), sortMode));
      }
      void runSync(user.id, 'manual');
  ...
  const pauseTimer = useCallback(
    async (task: Task) => {
      ...
      const updated = await updateTaskLocal(user.id, task.id, {timerStartedAt: null});
      if (!updated) {
        return;
      }
      setTasks(prev => sortTasks(prev.map(t => (t.id === task.id ? updated : t)), sortMode));
