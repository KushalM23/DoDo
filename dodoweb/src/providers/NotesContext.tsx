...
export function NotesProvider({children}: {children: React.ReactNode}) {
  ...
  const updateNote = useCallback(
    async (noteId: string, updates: UpdateNoteInput, options?: {sync?: boolean}) => {
      if (!user?.id) {
        return null;
      }
      ...
      setNotes(prev => sortNotes(prev.map(note => (note.id === noteId ? updated : note))));
  ...
  const value = useMemo<NotesContextValue>(
    ...
    [addNote, initialized, loading, notes, refresh, removeNote, syncNow, togglePin, updateNote],
  );
