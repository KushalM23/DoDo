...
export function RegisterScreen({ navigation }: Props) {
  ...
  async function onSubmit() {
    if (!name.trim()) { showAlert("Name required", "Please enter your name."); return; }
    setBusy(true);
    try {
      ...
      showAlert("Registration failed", e instanceof Error ? e.message : "Unknown error");
      ...
      >
        ...
        <View style={styles.fields}>
          {(["name", "email", "password"] as const).map((f) => (
            <View key={f} style={[styles.field, focused === f && styles.fieldFocused]}>
              <Text style={styles.fieldLabel}>{f === "name" ? "Your Name" : f === "email" ? "Email" : "Password"}</Text>
              <TextInput
                secureTextEntry={f === "password"}
                autoCapitalize={f === "name" ? "words" : "none"}
                keyboardType={f === "email" ? "email-address" : "default"}
                placeholder={f === "name" ? "Alex" : f === "email" ? "you@example.com" : "••••••••"}
                placeholderTextColor={colors.mutedText}
                style={styles.fieldInput}
                value={f === "name" ? name : f === "email" ? email : password}
                onChangeText={f === "name" ? setName : f === "email" ? setEmail : setPassword}
        ...
        <Animated.View style={{ transform: [{ scale: btnScale }] }}>
          ...
          >
            <Text style={styles.createText}>{busy ? "Creating…" : "Create Account"}</Text>
          </Pressable>
        </Animated.View>
...
const createStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    content: {
      flexGrow: 1, paddingHorizontal: 28, paddingTop: 60,
      paddingBottom: 48, justifyContent: "space-between",
    },
    heroSection: { gap: 6, marginBottom: 8 },
    backBtn: { marginBottom: 16 },
    backText: { fontSize: 15, fontWeight: "600", color: c.mutedText, fontFamily: fonts.bodySemiBold },
    headline: {
      fontSize: 64, fontWeight: "900", fontFamily: fonts.heading, color: c.text,
      letterSpacing: -3.5, lineHeight: 68,
    },
    tagline: { fontSize: 18, fontWeight: "600", color: c.mutedText, fontFamily: fonts.bodySemiBold, letterSpacing: -0.3 },
    ...
    fieldInput: { fontSize: 18, fontWeight: "600", color: c.text, fontFamily: fonts.bodySemiBold },
    createBtn: {
      backgroundColor: c.accent, borderRadius: 22,
      paddingVertical: 20, alignItems: "center", justifyContent: "center",
      marginTop: 32, shadowColor: c.accent,
      shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.45,
      shadowRadius: 24, elevation: 10,
    },
    createText: { color: "#fff", fontSize: 20, fontWeight: "800", fontFamily: fonts.bodyBold, letterSpacing: -0.3 },
    login: { flexDirection: "row", justifyContent: "center", marginTop: 24 },
    loginText: { color: c.mutedText, fontSize: 15, fontFamily: fonts.body },
    loginLink: { color: c.accent, fontSize: 15, fontWeight: "700", fontFamily: fonts.bodyBold },
  });
