...
export function HoldToConfirmButton({
  ...
  const borderRadius = square
    ? Math.max(radii.xl, Math.round(size * 0.28))
    : radii.lg;
  ...
  const cornerTopRight = Math.max(
    0,
    Math.min(1, (progress - 0.25) / cornerTransition),
  );
  const cornerBottomRight = Math.max(
    0,
    Math.min(1, (progress - 0.5) / cornerTransition),
  );
  const cornerBottomLeft = Math.max(
    0,
    Math.min(1, (progress - 0.75) / cornerTransition),
  );
    ...
    <Pressable
      ...
      {progressStyle === 'fill' ? (
        <View style={styles.progressTrackVertical}>
          <View
            style={[
              ...
              {height: `${progress * 100}%`, backgroundColor: mutedFillColor},
            ]}
          />
        </View>
      ) : (
        <View pointerEvents="none" style={styles.borderProgressWrap}>
          ...
          <View
            style={[
              styles.cornerDotTopLeft,
              {
                width: cornerSize,
                height: cornerSize,
                backgroundColor: resolvedFillColor,
                opacity: cornerTopLeft,
              },
            ]}
          />
          <View
            style={[
              styles.cornerDotTopRight,
              {
                width: cornerSize,
                height: cornerSize,
                backgroundColor: resolvedFillColor,
                opacity: cornerTopRight,
              },
            ]}
          />
          <View
            style={[
              styles.cornerDotBottomRight,
              {
                width: cornerSize,
                height: cornerSize,
                backgroundColor: resolvedFillColor,
                opacity: cornerBottomRight,
              },
            ]}
          />
          <View
            style={[
              styles.cornerDotBottomLeft,
              {
                width: cornerSize,
                height: cornerSize,
                backgroundColor: resolvedFillColor,
                opacity: cornerBottomLeft,
              },
            ]}
          />
        </View>
      ...
      {iconName ? (
        <AppIcon name={iconName} size={22} color={contentColor} />
      ) : null}
      {label ? (
        <Text style={[styles.label, {color: contentColor}]}>{label}</Text>
      ) : null}
      {shouldShowHint ? (
        <Text style={styles.hint}>{formatHoldHint(holdDurationMs)}</Text>
      ) : null}
    </Pressable>
  );
}
